...
file_names_norm <- list.files("results/normal2")
...
mk_dataset <- function(index){
  ...
  obj_names <- lapply(paste0("results/normal2/", list_of_file_names), load, .GlobalEnv)
  res1 <- bind_rows(bucher.results, maic.results, bayes_gcomp.results, CE.results, mis_bayes.results) %>% 
      mutate(se = sqrt(var),
...
norm_se_sum %>% 
  # build the printable "estimate (mcse)" value
  mutate(value = sprintf("%.3f(%.5f)", est, mcse)) %>% 
  
  dplyr::select(stat, N_AC, meanX_AC, value, method) %>% 
  tidyr::pivot_wider(names_from = method,
              values_from = value) %>% 
  
  dplyr::select(stat, N_AC, meanX_AC,          
                Bucher,                        
                Bayes_Gcomp,
                `G-MAIC`,
                MAIC,
                `(Mis)Bayes_Gcomp`) %>% 
  arrange(N_AC, meanX_AC, stat) %>% 
  kable(format = "html",
        col.names = c("Statistics", "Sample size", "IPD location",
                      "Bucher", "Bayes Gcomp", "G-MAIC",
                      "MAIC", "Bayes Gcomp(mis-spec)")) %>% 
