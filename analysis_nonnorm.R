...
file_names_nonnorm <- list.files("results/non_norm2")
...
mk_dataset <- function(index){
  ...
  obj_names <- lapply(paste0("results/non_norm2/", list_of_file_names), load, .GlobalEnv)
...
se_nonnorm_summary %>% 
  ...
  dplyr::select(stat, N_AC, X1_BC, value, method) %>% 
  tidyr::pivot_wider(names_from = method,
                     values_from = value) %>% 
  
  dplyr::select(stat, N_AC, X1_BC,          
                Bucher,                        
                Bayes_Gcomp,
                `G-MAIC`,
                MAIC,
                `(Mis)Bayes_Gcomp`) %>% 
  arrange(N_AC, X1_BC, stat) %>% 
  kable(format = "html",
